...
<xsl:template match="//infoObject" mode="krextor:main">
	...
	<xsl:call-template name="krextor:create-resource">
    	<xsl:with-param name="subject" select="concat('&cris;organization/',$id)"/>
    	
    	<xsl:with-param name="properties">
            <krextor:property uri="&rdf;type" object="&foaf;Organization"/>
            <krextor:property uri="http://vocab.lodum.de/helper/orgaID" value="{$id}"/>
...
<xsl:template name="key-value-rules" match="//infoObject/attribute" mode="krextor:main">
	<xsl:variable name="street" select="../attribute[@name='Street']/data"/>
	<xsl:variable name="postcode" select="../attribute[starts-with(@name,'Postal')]/data"/>
	<xsl:variable name="city" select="../attribute[@name='City']/data"/>
	<xsl:variable name="country" select="../attribute[@name='Country']/additionalInfo"/>
    ...
    <xsl:choose>
		...
		<xsl:when test="$varName='Description' and $varLang='1' and string($data)">
	    	<xsl:call-template name="krextor:add-literal-property">
	        	<xsl:with-param name="property" select="concat('&dc;','description')"/>
	        	<xsl:with-param name="language" select="'de'" />
	        	<xsl:with-param name="object" select="$data"/>
	    	</xsl:call-template>
		</xsl:when>
		...
		<xsl:when test="string($street) and string($postcode)">
			 <xsl:call-template name="krextor:add-uri-property">
	        	<xsl:with-param name="property" select="concat('&vcard;','adr')"/>
	        	<xsl:with-param name="object" select="concat(concat(concat(concat('&cris;','addresses/'),$postcode),'/'),translate(translate(translate(translate(translate(translate(normalize-space($street),'&#252;','ue'),'&#223;','ss'),'&#246;','oe'),'&#228;','ae'),' ',''),'.',''))"/> 
	    	</xsl:call-template>
			  <call-template name="krextor:create-resource">
			  		<with-param  name="subject" select="concat(concat(concat(concat('&cris;','addresses/'),$postcode),'/'),translate(translate(translate(translate(translate(translate(normalize-space($street),'&#252;','ue'),'&#223;','ss'),'&#246;','oe'),'&#228;','ae'),' ',''),'.',''))"/> 
			    	<with-param name="type" select="'&vcard;Address'"/>
			    	<xsl:with-param name="properties">
            			<krextor:property uri="&vcard;street-address" value="{$street}" datatype="&xsd;string"/>
            			<krextor:property uri="&vcard;postal-code" value="{$postcode}" datatype="&xsd;string"/>
            			<krextor:property uri="&vcard;country-name" value="{$country}" datatype="&xsd;string"/>
            			<krextor:property uri="&vcard;region" value="{$city}"  datatype="&xsd;string"/>
        			</xsl:with-param>
			  </call-template>
